/**
 * Curriki report namespace
 */
M.gradereport_curriki = {
    /**
     * @namespace M.gradereport_curriki
     * @param {Object} reports A collection of classes used by the curriki report module
     */
    classes : {},
    /**
     * Instantiates a new curriki report
     *
     * @function
     * @param {YUI} Y
...
/**
 * Initialises the JavaScript for the gradebook curriki report
 *
 * The functions fall into 3 groups:
 * M.gradereport_curriki.classes.ajax Used when editing is off and fields are dynamically added and removed
 * M.gradereport_curriki.classes.existingfield Used when editing is on meaning all fields are already displayed
 * M.gradereport_curriki.classes.report Common to both of the above
 *
 * @class report
 * @constructor
 * @this {M.gradereport_curriki}
 * @param {YUI} Y
 ...
 */
M.gradereport_curriki.classes.report = function(Y, cfg, items, users, feedback, grades) {
    this.Y = Y;
    ...
    if (this.ajaxenabled) {
        this.ajax = new M.gradereport_curriki.classes.ajax(this, cfg);
    }
};
/**
 * Extend the report class with the following methods and properties
 */
M.gradereport_curriki.classes.report.prototype.table = null;           // YUI Node for the reports main table
M.gradereport_curriki.classes.report.prototype.items = [];             // Array containing grade items
M.gradereport_curriki.classes.report.prototype.users = [];             // Array containing user information
M.gradereport_curriki.classes.report.prototype.feedback = [];          // Array containing feedback items
M.gradereport_curriki.classes.report.prototype.ajaxenabled = false;    // True is AJAX is enabled for the report
M.gradereport_curriki.classes.report.prototype.ajax = null;            // An instance of the ajax class or null
/**
 ...
 * @return {Object}
 */
M.gradereport_curriki.classes.report.prototype.get_cell_info = function(arg) {
 ...
 * @return {Bool}
 */
M.gradereport_curriki.classes.report.prototype.update_feedback = function(userid, itemid, newfeedback) {
 ...
 * @class ajax
 * @constructor
 * @this {M.gradereport_curriki.ajax}
 * @param {M.gradereport_curriki.classes.report} report
 * @param {Object} cfg
 */
M.gradereport_curriki.classes.ajax = function(report, cfg) {
    this.report = report;
    ...
    if (!report.isediting) {
        ...
        for (var userid in report.users) {
            ...
            for (var itemid in report.items) {
                this.existingfields[userid][itemid] = new M.gradereport_curriki.classes.existingfield(this, userid, itemid);
            }
        }
        // Disable the Update button as we're saving using ajax.
        submitbutton = this.report.Y.one('#currikisubmit');
 ...
 */
M.gradereport_curriki.classes.ajax.prototype.report = null;                  // A reference to the report class this object will use
M.gradereport_curriki.classes.ajax.prototype.courseid = null;                // The id for the course being viewed
M.gradereport_curriki.classes.ajax.prototype.feedbacktrunclength = null;     // The length to truncate feedback to
M.gradereport_curriki.classes.ajax.prototype.studentsperpage = null;         // The number of students shown per page
M.gradereport_curriki.classes.ajax.prototype.showquickfeedback = null;       // True if feedback editing should be shown
M.gradereport_curriki.classes.ajax.prototype.current = null;                 // The field being currently editing
M.gradereport_curriki.classes.ajax.prototype.pendingsubmissions = [];        // Array containing pending IO transactions
M.gradereport_curriki.classes.ajax.prototype.scales = [];                    // An array of scales used in this report
/**
 * Makes a cell editable
 * @function
 * @this {M.gradereport_curriki.classes.ajax}
 */
M.gradereport_curriki.classes.ajax.prototype.make_editable = function(e) {
    var node = e;
    if (e.halt) {
        e.halt();
        node = e.target;
    ...
    switch (fieldtype) {
        case 'scale':
            this.current = new M.gradereport_curriki.classes.scalefield(this.report, node);
            break;
        case 'text':
            this.current = new M.gradereport_curriki.classes.feedbackfield(this.report, node);
            break;
        default:
            this.current = new M.gradereport_curriki.classes.textfield(this.report, node);
            break;
    }
    this.current.replace().attach_key_events();

    // Fire the global resized event for the gradereport_curriki to update the table row/column sizes.
    Y.Global.fire('moodle-gradereport_curriki:resized');
};
/**
 ...
 * @param {Event} e
 */
M.gradereport_curriki.classes.ajax.prototype.keypress_enter = function(e) {
 ...
 */
M.gradereport_curriki.classes.ajax.prototype.keypress_tab = function(e, ignoreshift) {
    e.preventDefault();
    var next = null;
    if (e.shiftKey) {
        if (ignoreshift) {
 ...
 * @this {M.gradereport_curriki.classes.ajax}
 */
M.gradereport_curriki.classes.ajax.prototype.keypress_arrows = function(e) {
    e.preventDefault();
    var next = null;
    switch (e.keyCode) {
        case 37:    // Left
 ...
 */
M.gradereport_curriki.classes.ajax.prototype.process_editable_field = function(next) {
 ...
 * @return {Y.Node}
 */
M.gradereport_curriki.classes.ajax.prototype.get_next_cell = function(cell) {
 ...
 * @return {Y.Node}
 */
M.gradereport_curriki.classes.ajax.prototype.get_prev_cell = function(cell) {
    var n = cell || this.current.node;
 ...
 * @return {Y.Node}
 */
M.gradereport_curriki.classes.ajax.prototype.get_above_cell = function(cell) {
    var n = cell || this.current.node;
 ...
 * @return {Y.Node}
 */
M.gradereport_curriki.classes.ajax.prototype.get_below_cell = function(cell) {
    var n = cell || this.current.node;
 ...
 */
M.gradereport_curriki.classes.ajax.prototype.submit = function(properties, values) {
    ...
    if (values.grade !== values.oldgrade) {
        this.pendingsubmissions.push({transaction:this.report.Y.io.queue(M.cfg.wwwroot+'/grade/report/curriki/ajax_callbacks.php', {
            method : 'POST',
 ...
 * @param {Mixed} args
 */
M.gradereport_curriki.classes.ajax.prototype.submission_outcome = function(tid, outcome, args) {
    // Parse the response as JSON
    try {
        outcome = this.report.Y.JSON.parse(outcome.responseText);
    } catch(e) {
        var message = M.util.get_string('ajaxfailedupdate', 'gradereport_curriki');
    ...
    // Quick reference for the curriki report
    var i = null;
    // Check the outcome
            ...
            if (outcome.row[i] && outcome.row[i].userid && outcome.row[i].itemid) {
                ...
                if (this.report.isediting) {
                    ...
                    if (!this.current || info.cell.get('id') != this.current.node.get('id')) {
                        ...
                        if (!this.current.has_changed() || confirm(M.util.get_string('ajaxfieldchanged', 'gradereport_curriki'))) {
 ...
 * @param {Y.Node} cell
 */
M.gradereport_curriki.classes.ajax.prototype.display_submission_error = function(message, cell) {
    var erroroverlay = new this.report.Y.Overlay({
        headerContent : '<div><strong class="error">'+M.util.get_string('ajaxerror', 'gradereport_curriki')+'</strong>  <em>'+M.util.get_string('ajaxclicktoclose', 'gradereport_curriki')+'</em></div>',
        bodyContent : message,
        visible : false,
        zIndex : 3
    });
 ...
 * @param {Int} itemid
 */
M.gradereport_curriki.classes.existingfield = function(ajax, userid, itemid) {
 ...
 * via prototyping
 */
M.gradereport_curriki.classes.existingfield.prototype.userid = null;
M.gradereport_curriki.classes.existingfield.prototype.itemid = null;
M.gradereport_curriki.classes.existingfield.prototype.editfeedback = false;
M.gradereport_curriki.classes.existingfield.prototype.grade = null;
M.gradereport_curriki.classes.existingfield.prototype.oldgrade = null;
M.gradereport_curriki.classes.existingfield.prototype.keyevents = [];
/**
 * Handles saving of changed on keypress
 *
 * @function
 * @this {M.gradereport_curriki.classes.existingfield}
 * @param {Event} e
 */
M.gradereport_curriki.classes.existingfield.prototype.keypress_enter = function(e) {
    e.preventDefault();
    this.submit();
};
/**
 ...
 */
M.gradereport_curriki.classes.existingfield.prototype.keypress_tab = function(e, ignoreshift) {
    e.preventDefault();
    var next = null;
    if (e.shiftKey) {
        if (ignoreshift) {
 ...
 * @param {Event} e
 */
M.gradereport_curriki.classes.existingfield.prototype.keypress_arrows = function(e) {
    e.preventDefault();
    var next = null;
    switch (e.keyCode) {
        case 37:    // Left
 ...
 * @param {Y.Node} node
 */
M.gradereport_curriki.classes.existingfield.prototype.move_focus = function(node) {
    if (node) {
 ...
 * @return {Bool}
 */
M.gradereport_curriki.classes.existingfield.prototype.has_changed = function() {
    if (this.grade) {
 ...
 * @this {M.gradereport_curriki.classes.existingfield}
 */
M.gradereport_curriki.classes.existingfield.prototype.submit = function() {
    if (!this.has_changed()) {
        return;
    }
 ...
 * @class textfield
 * @constructor
 * @this {M.gradereport_curriki.classes.textfield}
 * @param {M.gradereport_curriki.classes.report} report
 * @param {Y.Node} node
 */
M.gradereport_curriki.classes.textfield = function(report, node) {
    this.report = report;
    this.node = node;
 ...
 */
M.gradereport_curriki.classes.textfield.prototype.keyevents = [];
M.gradereport_curriki.classes.textfield.prototype.editable = false;
M.gradereport_curriki.classes.textfield.prototype.gradetype = null;
M.gradereport_curriki.classes.textfield.prototype.grade = null;
M.gradereport_curriki.classes.textfield.prototype.report = null;
M.gradereport_curriki.classes.textfield.prototype.node = null;
M.gradereport_curriki.classes.textfield.prototype.gradespam = null;
M.gradereport_curriki.classes.textfield.prototype.inputdiv = null;
M.gradereport_curriki.classes.textfield.prototype.editfeedback = false;
/**
 * Replaces the cell contents with the controls to enable editing
 *
 * @function
 * @this {M.gradereport_curriki.classes.textfield}
 * @return {M.gradereport_curriki.classes.textfield}
 */
M.gradereport_curriki.classes.textfield.prototype.replace = function() {
 ...
 * @return {Object}
 */
M.gradereport_curriki.classes.textfield.prototype.commit = function() {
 ...
 * @this {M.gradereport_curriki.classes.textfield}
 */
M.gradereport_curriki.classes.textfield.prototype.revert = function() {
 ...
 * @return {Mixed}
 */
M.gradereport_curriki.classes.textfield.prototype.get_grade = function() {
    if (this.editable) {
 ...
 * @param {Mixed} value
 */
M.gradereport_curriki.classes.textfield.prototype.set_grade = function(value) {
    if (!this.editable) {
        if (value == '-') {
            value = '';
        }
 ...
 * @return {String}
 */
M.gradereport_curriki.classes.textfield.prototype.get_feedback = function() {
    if (this.editable) {
        if (this.feedback) {
 ...
 * @param {Mixed} value
 */
M.gradereport_curriki.classes.textfield.prototype.set_feedback = function(value) {
    if (!this.editable) {
        if (this.feedback) {
 ...
 * @return {Bool}
 */
M.gradereport_curriki.classes.textfield.prototype.has_changed = function() {
 ...
 * @this {M.gradereport_curriki.classes.textfield}
 */
M.gradereport_curriki.classes.textfield.prototype.attach_key_events = function() {
 ...
 * @class feedbackfield
 * @constructor
 * @this {M.gradereport_curriki.classes.feedbackfield}
 * @param {M.gradereport_curriki.classes.report} report
 * @param {Y.Node} node
 */
M.gradereport_curriki.classes.feedbackfield = function(report, node) {
    this.report = report;
    this.node = node;
    this.gradespan = node.one('.gradevalue');
    this.inputdiv = this.report.Y.Node.create('<div></div>');
 ...
 * @return {Mixed}
 */
M.gradereport_curriki.classes.feedbackfield.prototype.get_grade = function() {
    return null;
};
 ...
 * @param {String} value
 */
M.gradereport_curriki.classes.feedbackfield.prototype.set_grade = function() {
    return;
};
 ...
 */
for (var i in M.gradereport_curriki.classes.textfield.prototype) {
    if (!M.gradereport_curriki.classes.feedbackfield.prototype[i]) {
        M.gradereport_curriki.classes.feedbackfield.prototype[i] = M.gradereport_curriki.classes.textfield.prototype[i];
    }
}
 ...
 * @class scalefield
 * @constructor
 * @inherits M.gradereport_curriki.classes.textfield
 * @base M.gradereport_curriki.classes.textfield
 * @this {M.gradereport_curriki.classes.scalefield}
 * @param {M.gradereport_curriki.classes.report} report
 * @param {Y.Node} node
 */
M.gradereport_curriki.classes.scalefield = function(report, node) {
    this.report = report;
    this.node = node;
    this.gradespan = node.one('.gradevalue');
    this.inputdiv = this.report.Y.Node.create('<div></div>');
    ...
    this.grade = this.report.Y.Node.create('<select type="text" class="text" name="ajaxgrade" /><option value="-1">'+
            M.util.get_string('ajaxchoosescale', 'gradereport_curriki')+'</option></select>');
 ...
 */
M.gradereport_curriki.classes.scalefield.prototype.scale = [];
/**
 ...
 */
M.gradereport_curriki.classes.scalefield.prototype.get_grade = function(){
    if (this.editable) {
 ...
 * @param {String} value
 */
M.gradereport_curriki.classes.scalefield.prototype.set_grade = function(value) {
    if (!this.editable) {
        if (value == '-') {
            value = '-1';
        }
 ...
 * @return {Bool}
 */
M.gradereport_curriki.classes.scalefield.prototype.has_changed = function() {
    if (!this.editable) {
        return false;
    }
 ...
 */
for (var i in M.gradereport_curriki.classes.textfield.prototype) {
    if (!M.gradereport_curriki.classes.scalefield.prototype[i]) {
        M.gradereport_curriki.classes.scalefield.prototype[i] = M.gradereport_curriki.classes.textfield.prototype[i];
    }
}
