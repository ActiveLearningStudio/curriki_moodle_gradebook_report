...
/**
 * Definition of the curriki report class
 *
...
/**
 * Class providing an API for the curriki report building and displaying.
 * @uses grade_report
 * @copyright 2007 Nicolas Connault
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class grade_report_curriki extends grade_report {
    /**
     * The final grades.
     * @var array $grades
     */
    ...
    public function __construct($courseid, $gpr, $context, $page=null, $sortitemid=null) {
        ...
        $this->pbarurl = new moodle_url('/grade/report/curriki/index.php', array('id' => $this->courseid));
    ...
    public function load_users_legacy() {
        global $CFG, $DB;
    ...
    public function load_final_grades() {
        ...
        if ($grades = $DB->get_records_sql($sql, $params)) {
            foreach ($grades as $grade000rec) {
                $grade = new grade_grade($grade000rec, false);
                if (!empty($allgradeitems[$grade000rec->itemid])) {
                    ...
                    $this->allgrades[$grade000rec->userid][$grade000rec->itemid] = $grade;
                }
                if (in_array($grade000rec->userid, $userids) and array_key_exists($grade000rec->itemid, $this->gtree->get_items())) { // some items may not be present!!
                    $this->grades[$grade000rec->userid][$grade000rec->itemid] = $grade;
                    $this->grades[$grade000rec->userid][$grade000rec->itemid]->grade_item = $this->gtree->get_item($grade000rec->itemid); // db caching
                }
            }
        }
    ...
    /**
     * Builds and returns the rows that will make up the left part of the curriki report
    ...
    public function get_left_rows_legacy($displayaverages) {
        global $CFG, $USER, $OUTPUT;
        ...
        $arrows = $this->get_sort_arrows_legacy($extrafields);
    ...
    public function get_right_rows_legacy($displayaverages) {
        ...
        $strgrade = $this->get_lang_string('grade');
        $strfeedback  = $this->get_lang_string("feedback");
        $arrows = $this->get_sort_arrows_legacy();
                            ...
                            if ($item->itemmodule === 'lti') {
                                ...
                                if ($modinfo) {
                                    ...
                                    if (!empty($instances[$itemmodule][$iteminstance])) {
                                        ...
                                        $activityUrl = new moodle_url('/mod/lti/view.php', array('id' => $cm->id));
                                        $currikiReportUrl = new moodle_url($this->baseurl, array('studentid' => $grade->userid, 'activityRedirectUrl' => urlencode($activityUrl)));
                                        $currikiReportLink = '<a href="'.$currikiReportUrl.'" id="action_link_'. $grade->grade_item->id . '-' . $grade->userid .'" class="action-icon" title="Grade analysis" aria-label="Grade analysis"><i class="icon fa fa-search-plus fa-fw " aria-hidden="true"></i></a>';
                                        $itemcell->text .= $currikiReportLink;
                                    }
                                }
                            }
                        }
    ...
    /**
     * Builds and returns the rows that will make up the right part of the curriki report
     * @param boolean $displayaverages whether to display average rows in the table
     * @return array Array of html_table_row objects
     */
    public function get_right_rows($displayaverages) {
        ...
        foreach ($this->users as $userid => $user) {
            ...
            foreach ($this->gtree->items as $itemid => $unused) {
                ...
                // MDL-11274
                // Hide grades in the curriki report if the current curriki doesn't have 'moodle/grade:viewhidden'
                if (!$this->canviewhidden and $grade->is_hidden()) {
                    if (!empty($CFG->grade_hiddenasdate) and $grade->get_datesubmitted() and !$item->is_category_item() and !$item->is_course_item()) {
                        // the problem here is that we do not have the time when grade value was modified, 'timemodified' is general modification date for grade_grades records
                        $itemcell->text = "<span class='datesubmitted'>" .
                ...
                if ($grade->is_overridden()) {
                    $itemcell->attributes['class'] .= ' overridden';
                    $itemcell->attributes['aria-label'] = get_string('overriddengrade', 'gradereport_curriki');
                }
                ...
                if ($item->needsupdate) {
                    ...
                    if ($item->scaleid && !empty($scalesarray[$item->scaleid])) {
                        ...
                        if ($quickgrading and $grade->is_editable()) {
                            ...
                            $gradelabel = $fullname . ' ' . $item->get_name(true);
                            $itemcell->text .= html_writer::label(
                                get_string('useractivitygrade', 'gradereport_curriki', $gradelabel), $attributes['id'], false,
                                    array('class' => 'accesshide'));
                            $itemcell->text .= html_writer::select($scaleopt, 'grade['.$userid.']['.$item->id.']', $gradeval, array(-1=>$nogradestr), $attributes);
                        } else if (!empty($scale)) {
                            $scales = explode(",", $scale->scale);
                        ...
                        if ($quickgrading and $grade->is_editable()) {
                            ...
                            $gradelabel = $fullname . ' ' . $item->get_name(true);
                            $itemcell->text .= '<label class="accesshide" for="grade_'.$userid.'_'.$item->id.'">'
                                          .get_string('useractivitygrade', 'gradereport_curriki', $gradelabel).'</label>';
                            $itemcell->text .= '<input size="6" tabindex="' . $tabindices[$item->id]['grade']
                                          . '" type="text" class="text" title="'. $strgrade .'" name="grade['
                                          .$userid.'][' .$item->id.']" id="grade_'.$userid.'_'.$item->id.'" value="'.$value.'" />';
                        } else {
                    ...
                    if ($showquickfeedback and $grade->is_editable()) {
                        $feedbacklabel = $fullname . ' ' . $item->get_name(true);
                        $itemcell->text .= '<label class="accesshide" for="feedback_'.$userid.'_'.$item->id.'">'
                                      .get_string('useractivityfeedback', 'gradereport_curriki', $feedbacklabel).'</label>';
                        $itemcell->text .= '<input class="quickfeedback" tabindex="' . $tabindices[$item->id]['feedback'].'" id="feedback_'.$userid.'_'.$item->id
                                      . '" size="6" title="' . $strfeedback . '" type="text" name="feedback['.$userid.']['.$item->id.']" value="' . s($grade->feedback) . '" />';
                    }
        ...
        $module = array(
            'name'      => 'gradereport_curriki',
            'fullpath'  => '/grade/report/curriki/module.js',
            'requires'  => array('base', 'dom', 'event', 'event-mouseenter', 'event-key', 'io-queue', 'json-parse', 'overlay')
        );
        $PAGE->requires->js_init_call('M.gradereport_curriki.init_report', $jsarguments, false, $module);
        $PAGE->requires->strings_for_js(array('addfeedback', 'feedback', 'grade'), 'grades');
        $PAGE->requires->strings_for_js(array('ajaxchoosescale', 'ajaxclicktoclose', 'ajaxerror', 'ajaxfailedupdate', 'ajaxfieldchanged'), 'gradereport_curriki');
        if (!$enableajax && !empty($USER->editing)) {
            $PAGE->requires->js_call_amd('core_form/changechecker', 'watchFormById', ['gradereport_curriki']);
        }
    ...
    public function get_grade_table($displayaverages = false) {
        ...
        if (class_exists('\core_user\fields')) {
            ...
            $leftrows = $this->get_left_rows_legacy($displayaverages);
            $rightrows = $this->get_right_rows_legacy($displayaverages);
        }
        
        $html = '';

        $fulltable = new html_table();
        $fulltable->attributes['class'] = 'gradereport-curriki-table';
        $fulltable->id = 'user-grades';
        $fulltable->caption = get_string('summarycurriki', 'gradereport_curriki');
    ...
    /**
     * Builds and return the header for the row of ranges, for the left part of the curriki report.
     * @param array $rows The Array of rows for the left part of the report
     * @param int $colspan The number of columns this cell has to span
     * @return array Array of rows for the left part of the report
     */
    ...
    /**
     * Builds and return the headers for the rows of averages, for the left part of the curriki report.
     * @param array $rows The Array of rows for the left part of the report
     * @param int $colspan The number of columns this cell has to span
    ...
    /**
     * Builds and return the row of icons when editing is on, for the right part of the curriki report.
    ...
    /**
     * Builds and return the row of ranges for the right part of the curriki report.
     * @param array $rows The Array of rows for the right part of the report
     * @return array Array of rows for the right part of the report
     */
    ...
    /**
     * Builds and return the row of averages for the right part of the curriki report.
     ...
     * with the icons needed for the curriki report.
     *
     * @param array $element
     * @return string HTML
     */
    ...
    protected static function get_collapsed_preferences($courseid) {
        if ($collapsed = get_user_preferences('grade_report_curriki_collapsed_categories'.$courseid)) {
        ...
        $collapsedall = [];
        $oldprefexists = false;
        if (($oldcollapsedpref = get_user_preferences('grade_report_curriki_collapsed_categories')) !== null) {
            $oldprefexists = true;
            if ($collapsedall = unserialize_array($oldcollapsedpref)) {
                ...
                if (!empty($collapsed['aggregatesonly']) || !empty($collapsed['gradesonly'])) {
                    ...
                    if (!empty($collapsedall['aggregatesonly']) || !empty($collapsedall['gradesonly'])) {
                        set_user_preference('grade_report_curriki_collapsed_categories', serialize($collapsedall));
                    }
                }
            }
        }
        ...
        if ($oldprefexists &&
            ...
            unset_user_preference('grade_report_curriki_collapsed_categories');
        }
    ...
    protected static function set_collapsed_preferences($courseid, $collapsed) {
        ...
        if (!empty($collapsed['aggregatesonly']) || !empty($collapsed['gradesonly'])) {
            set_user_preference('grade_report_curriki_collapsed_categories'.$courseid, json_encode($collapsed));
        } else {
            unset_user_preference('grade_report_curriki_collapsed_categories'.$courseid);
        }
    }
    ...
    public static function do_process_action($target, $action, $courseid = null) {
        ...
        if (!$courseid) {
            debugging('Function grade_report_curriki::do_process_action() now requires additional argument courseid',
                DEBUG_DEVELOPER);
    ...
    public function get_sort_arrows_legacy(array $extrafields = array()) {
        global $OUTPUT, $CFG;
        $arrows = array();
